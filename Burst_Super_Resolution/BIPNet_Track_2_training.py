...
class BurstSR_Network:
    def __init__(self, args):
        ...
        self.aligned_psnr_fn = AlignedPSNR(
            alignment_net=alignment_net, boundary_ignore=40
        )
    ...
    def train(self):
        ...
        if os.path.exists(args.model_dir + "./BIPNet.ckpt"):
            ...
            model = (
                BIPNet().load_from_checkpoint(args.model_dir + "./BIPNet.ckpt").cuda()
            )
        else:
        ...
        for epoch in range(epochs):
            ...
            with tqdm(
                ...
                for i, data in enumerate(train_data_loader):
                    ...
                    loss = self.aligned_L1_loss(
                        preds, labels, burst
                    ) + 0.0095 * self.AlignedLPIPS_loss(preds, labels, burst)
                    ...
                    if i % (len(train_data_loader) // 4) == 0 and i > 0:
                        ...
                        print(
                            "[epoch %d it %d PSNR: %.6f --- best_epoch %d best_iter %d Best_PSNR %.6f]"
                            % (epoch, i, PSNR, best_epoch, best_iter, best_PSNR)
                        )
